...
class GlyphMap {
    ...
    // Headless-safe FontRenderContext — does NOT call getDefaultScreenDevice()
    private static final FontRenderContext HEADLESS_FRC =
            new FontRenderContext(new AffineTransform(), true, false);

    // Headless-safe ascent calculation using FontRenderContext instead of FontMetrics
    private int getAscent(Font f) {
        // Use LineMetrics which works in headless mode
        java.awt.font.LineMetrics lm = f.getLineMetrics("Ag", HEADLESS_FRC);
    ...
    public void generate() {
        ...
        while (generatedChars <= range) {
            ...
            Font f = getFontForGlyph(currentChar);
            // Use HEADLESS_FRC — safe on Android/headless environments
            Rectangle2D stringBounds = f.getStringBounds(String.valueOf(currentChar), HEADLESS_FRC);
        ...
        for (Glyph glyph : glyphs1) {
            Font f = getFontForGlyph(glyph.value());
            g2d.setFont(f);
            // Use headless-safe ascent instead of g2d.getFontMetrics().getAscent()
            int ascent = getAscent(f);
            g2d.drawString(String.valueOf(glyph.value()), glyph.u(), glyph.v() + ascent);
