...
class GlyphMap {
    ...
    public void generate() {
        ...
        // IMPORTANT: keep FRACTIONALMETRICS ON (the default) here. Setting it to OFF makes the JDK's
        // FontDesignMetrics use its legacy "device-dependent" metrics cache, which internally still
        // calls GraphicsEnvironment.getDefaultScreenDevice() regardless of which Graphics2D you measure
        // with - and that throws HeadlessException on headless/AWT-emulated setups like PojavLauncher's
        // caciocavallo on Android. Fractional (vector) metrics avoid that lookup entirely.
        metricsG2D.setRenderingHint(RenderingHints.KEY_FRACTIONALMETRICS, RenderingHints.VALUE_FRACTIONALMETRICS_ON);
        ...
        // Same reasoning as above: FRACTIONALMETRICS must stay ON, otherwise g2d.getFontMetrics()
        // below (used only to get the ascent for vertical placement) crashes the same way.
