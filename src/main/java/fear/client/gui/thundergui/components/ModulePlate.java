package fear.client.gui.thundergui.components;
...
import fear.client.gui.thundergui.ThunderGui;
import fear.client.setting.impl.Bind;
import fear.client.utility.render.Render2DEngine;
import fear.client.utility.render.animation.AnimationUtility;
    ...
    public void render(MatrixStack stack, int MouseX, int MouseY) {
        ...
        if (module.isOn()) {
            Render2DEngine.drawGradientRound(stack, posX + 1, posY, 89, 30, 4f,
                    Render2DEngine.applyOpacity(FearClientGui.onColor1.getValue().getColorObject(), getFadeFactor()),
                    Render2DEngine.applyOpacity(FearClientGui.onColor1.getValue().getColorObject(), getFadeFactor()),
                    Render2DEngine.applyOpacity(FearClientGui.onColor2.getValue().getColorObject(), getFadeFactor()),
                    Render2DEngine.applyOpacity(FearClientGui.onColor2.getValue().getColorObject(), getFadeFactor()));
        } else {
        ...
        if (listening_bind) {
            ...
            if (!holdbind) {
                Render2DEngine.drawRound(stack, posX + 6, posY + 6, 38, 8, 2, Render2DEngine.injectAlpha(FearClientGui.onColor1.getValue().getColorObject(), 170));
                ...
                Render2DEngine.drawRound(stack, posX + 6, posY + 16, 38, 8, 2, Render2DEngine.injectAlpha(FearClientGui.onColor1.getValue().getColorObject(), 170));
