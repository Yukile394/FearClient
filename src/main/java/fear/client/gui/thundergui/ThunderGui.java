...
public class ThunderGui extends Screen {
    ...
    private final int main_width = 460;
    ...
    public void renderGui(DrawContext context, int mouseX, int mouseY, float partialTicks) {
        ...
        Render2DEngine.drawRect(context.getMatrices(), main_posX, main_posY, main_width, main_height, FearClientGui.getColorByTheme(0));

        // Плита с лого / Main GUI logo
        Render2DEngine.drawRect(context.getMatrices(), main_posX + 5, main_posY + 5, 90, 30, FearClientGui.getColorByTheme(1));

        context.getMatrices().push();
        context.getMatrices().scale(0.85f, 0.85f, 1);
        context.getMatrices().translate((main_posX + 10) / 0.85, (main_posY + 15) / 0.85, 0);
        FontRenderers.thglitch.drawString(context.getMatrices(), "FEAR CLIENT", 0, 0, FearClientGui.getColorByTheme(2).getRGB());
        context.getMatrices().translate(-(main_posX + 10) / 0.85, -(main_posY + 15) / 0.85, 0);
        context.getMatrices().scale(1, 1, 1);
        context.getMatrices().pop();

        FontRenderers.settings.drawString(context.getMatrices(), "recode v" + FearClient.VERSION, main_posX + 91 - (FontRenderers.settings.getStringWidth("recode v" + FearClient.VERSION)), main_posY + 30, FearClientGui.getColorByTheme(3).getRGB());

        // Левая плита под категриями
        Render2DEngine.drawRect(context.getMatrices(), main_posX + 5, main_posY + 40, 90, 120, FearClientGui.getColorByTheme(4));

        // Выбор между CfgManager и FriendManager
        if (currentMode == CurrentMode.Modules) {
            Render2DEngine.drawRect(context.getMatrices(), main_posX + 20, main_posY + 195, 60, 20, FearClientGui.getColorByTheme(4));
        } else if (currentMode == CurrentMode.CfgManager) {
            Render2DEngine.draw2DGradientRect(context.getMatrices(), main_posX + 20, main_posY + 195, main_posX + 20 + 60, main_posY + 195 + 20, FearClientGui.getColorByTheme(4), FearClientGui.getColorByTheme(4), FearClientGui.getColorByTheme(5), FearClientGui.getColorByTheme(5));
        } else {
            Render2DEngine.draw2DGradientRect(context.getMatrices(), main_posX + 20, main_posY + 195, main_posX + 20 + 60, main_posY + 195 + 20, FearClientGui.getColorByTheme(5), FearClientGui.getColorByTheme(5), FearClientGui.getColorByTheme(4), FearClientGui.getColorByTheme(4));
        }

        Render2DEngine.drawRect(context.getMatrices(), main_posX + 49.5f, main_posY + 197, 1, 16, FearClientGui.getColorByTheme(6));

        FontRenderers.mid_icons.drawString(context.getMatrices(), "u", main_posX + 20, main_posY + 196, currentMode == CurrentMode.CfgManager ? FearClientGui.getColorByTheme(2).getRGB() : new Color(0x8D8D8D).getRGB());
        FontRenderers.mid_icons.drawString(context.getMatrices(), "v", main_posX + 54, main_posY + 197, currentMode == CurrentMode.FriendManager ? FearClientGui.getColorByTheme(2).getRGB() : new Color(0x8D8D8D).getRGB());
        ...
        if (first_open) {
            category_animation = 1;
            Render2DEngine.drawRect(context.getMatrices(), (float) (main_posX + 8), (float) CategoryY + slider_y, 84, 15, FearClientGui.getColorByTheme(7));
            first_open = false;
        } else {
            if (currentMode == CurrentMode.Modules)
                Render2DEngine.drawRect(context.getMatrices(), (float) (main_posX + 8), (float) (Render2DEngine.interpolate(CategoryY, prevCategoryY, category_animation)) + slider_y, 84, 15, FearClientGui.getColorByTheme(7));
        }
        ...
        if (selected_plate != null) {
            if (currentMode == CurrentMode.Modules)
                Render2DEngine.drawRect(context.getMatrices(), (float) Render2DEngine.interpolate(main_posX + 200, selected_plate.getPosX(), settings_animation), (float) Render2DEngine.interpolate(main_posY + 40, selected_plate.getPosY(), settings_animation), (float) Render2DEngine.interpolate(255, 90, settings_animation), (float) Render2DEngine.interpolate(main_height - 45, 30, settings_animation), FearClientGui.getColorByTheme(7));
        }
        ...
        if (currentMode != CurrentMode.Modules) {
            ...
            Render2DEngine.addWindow(context.getMatrices(), (float) Render2DEngine.interpolate(main_posX + 80, main_posX + 200, manager_animation), main_posY + 39, (float) Render2DEngine.interpolate(459, 195, manager_animation) + main_posX + 36, (float) main_height + main_posY - 3, 1d);

            Render2DEngine.drawRect(context.getMatrices(), main_posX + 100, (float) main_posY + 40, (float) 355, (float) main_height - 44, FearClientGui.getColorByTheme(7));
            ...
            Render2DEngine.draw2DGradientRect(context.getMatrices(), main_posX + 102, main_posY + 34, main_posX + 453, main_posY + 60, new Color(25, 20, 30, 0), FearClientGui.getColorByTheme(7), new Color(25, 20, 30, 0), new Color(37, 27, 41, 245));
            Render2DEngine.draw2DGradientRect(context.getMatrices(), main_posX + 102, main_posY + main_height - 35, main_posX + 453, main_posY + main_height, FearClientGui.getColorByTheme(7), new Color(25, 20, 30, 0), FearClientGui.getColorByTheme(7), new Color(37, 27, 41, 0));
            Render2DEngine.popWindow();
        }

        Render2DEngine.addWindow(context.getMatrices(), main_posX + 79, main_posY + 35, main_posX + 456 + 40, main_posY + main_height, 1d);
        ...
        Render2DEngine.drawRect(context.getMatrices(), main_posX + 100, main_posY + 5, 355, 30, new Color(25, 20, 30, 250));

        // Конфиг
        if (isHoveringItem(main_posX + 105, main_posY + 14, 11, 11, mouseX, mouseY)) {
            Render2DEngine.drawRect(context.getMatrices(), main_posX + 105, main_posY + 14, 11, 11, new Color(68, 49, 75, 250));
        } else {
            Render2DEngine.drawRect(context.getMatrices(), main_posX + 105, main_posY + 14, 11, 11, new Color(52, 38, 58, 250));
        }
        ...
        // Поиск
        Render2DEngine.drawRect(context.getMatrices(), main_posX + 310, main_posY + 15, 140, 10, new Color(52, 38, 58, 250));
        if (currentMode == CurrentMode.Modules)
            FontRenderers.icons.drawString(context.getMatrices(), "s", main_posX + 438, main_posY + 18, searching ? new Color(0xCBFFFFFF, true).getRGB() : new Color(0x83FFFFFF, true).getRGB());

        if (isHoveringItem(main_posX + 310, main_posY + 15, 140, 20, mouseX, mouseY)) {
            Render2DEngine.addWindow(context.getMatrices(), main_posX + 250, main_posY + 15, main_posX + 250 + 140, main_posY + 15 + 10, 1);
            //   GL11.glPushMatrix();
            Render2DEngine.drawRect(context.getMatrices(), main_posX + 250, main_posY + 15, 140, 10, new Color(84, 63, 94, 36));
            // Stencil.write(false);
        ...
        if (currentMode == CurrentMode.Modules)
            FontRenderers.modules.drawString(context.getMatrices(), search_string, main_posX + 312, main_posY + 19, searching ? new Color(0xCBFFFFFF, true).getRGB() : new Color(0x83FFFFFF, true).getRGB());
        if (currentMode == CurrentMode.CfgManager) {
            FontRenderers.modules.drawString(context.getMatrices(), config_string, main_posX + 312, main_posY + 19, listening_config ? new Color(0xCBFFFFFF, true).getRGB() : new Color(0x83FFFFFF, true).getRGB());
            Render2DEngine.drawRect(context.getMatrices(), main_posX + 428, main_posY + 17, 20, 6, isHoveringItem(main_posX + 368, main_posY + 17, 20, 6, mouseX, mouseY) ? new Color(59, 42, 63, 194) : new Color(33, 23, 35, 194));
            FontRenderers.modules.drawCenteredString(context.getMatrices(), "+", main_posX + 438, main_posY + 16, FearClientGui.getColorByTheme(2).getRGB());
        }
        if (currentMode == CurrentMode.FriendManager) {
            FontRenderers.modules.drawString(context.getMatrices(), friend_string, main_posX + 312, main_posY + 19, listening_friend ? new Color(0xCBFFFFFF, true).getRGB() : new Color(0x83FFFFFF, true).getRGB());
            Render2DEngine.drawRect(context.getMatrices(), main_posX + 368, main_posY + 17, 20, 6, isHoveringItem(main_posX + 368, main_posY + 17, 20, 6, mouseX, mouseY) ? new Color(59, 42, 63, 194) : new Color(33, 23, 35, 194));
            FontRenderers.modules.drawCenteredString(context.getMatrices(), "+", main_posX + 378, main_posY + 16, FearClientGui.getColorByTheme(2).getRGB());
        }
        ...
        float scissorX2 = Math.max((float) Render2DEngine.interpolate(455, 90, settings_animation) + main_posX, main_posX + 265) + 40;
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int clickedButton) {
        ...
        if (isHoveringItem(main_posX + 105, main_posY + 14, 11, 11, (float) mouseX, (float) mouseY)) {
            try {
                net.minecraft.util.Util.getOperatingSystem().open(new File("FearClientRecode/configs/").toURI());
            } catch (Exception e) {
        ...
        if (isHoveringItem(main_posX + 310, main_posY + 15, 110, 10, (float) mouseX, (float) mouseY) && currentMode == CurrentMode.CfgManager) {
    ...
    public void keyTyped(String typedChar, int keyCode) throws IOException {
        if (FearClient.currentKeyListener != FearClient.KeyListening.Sliders && FearClient.currentKeyListener != FearClient.KeyListening.ThunderGui)
            return;
    ...
    public boolean mouseScrolled(double mouseX, double mouseY, double horizontalAmount, double verticalAmount) {
        ...
        if (scroll_lock) {
            ...
            if (isHoveringItem(main_posX + 200, main_posY + 40, main_posX + 455, main_posY - 5 + main_height, (float) mouseX, (float) mouseY))
                scroll += dWheel * FearClientGui.scrollSpeed.getValue();
            else {
                components.forEach(component -> component.scrollElement(dWheel * FearClientGui.scrollSpeed.getValue()));
            }
            configs.forEach(component -> component.scrollElement(dWheel * FearClientGui.scrollSpeed.getValue()));
            friends.forEach(component -> component.scrollElement(dWheel * FearClientGui.scrollSpeed.getValue()));
        }
