...
import fear.client.events.impl.PostPlayerUpdateEvent;
...
public class Timer extends Module {
    ...
    public void onUpdate() {
        switch (mode.getValue()) {
            case Normal -> FearClient.TICK_TIMER = speed.getValue();
            case Matrix -> {
                ...
                FearClient.TICK_TIMER = Math.max(speed.getValue(), 1f);
            ...
            case Grim -> {
                if (energy <= 0 || !isKeyPressed(boostKey) || FearClient.core.getSetBackTime() < 2000) {
                    FearClient.TICK_TIMER = 1f;
                    return;
                }
    ...
    public void onPacketReceive(PacketEvent.Receive e) {
        if (mode.is(Mode.Grim)) {
            if (e.getPacket() instanceof CommonPingS2CPacket
                    && FearClient.core.getSetBackTime() > 2000) {
