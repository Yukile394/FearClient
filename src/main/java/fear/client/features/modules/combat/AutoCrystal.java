...
import fear.client.core.manager.world.CrystalManager;
import fear.client.events.impl.*;
...
import fear.client.setting.impl.Bind;
...
import fear.client.utility.TickTimer;
import fear.client.utility.Timer;
import fear.client.utility.world.ExplosionUtility;
import fear.client.utility.math.MathUtility;
import fear.client.utility.math.PredictUtility;
...
public class AutoCrystal extends Module {
    ...
    /*   RENDER   */
    public final Setting<Aura.ESP> targetEsp = new Setting<>("TargetESP", Aura.ESP.FearClient, v -> page.is(Pages.Render));
    public final Setting<SettingGroup> espGroup = new Setting<>("ESPSettings", new SettingGroup(false, 0), v -> targetEsp.is(Aura.ESP.FearClientV2) && page.is(Pages.Render));
    public final Setting<Integer> espLength = new Setting<>("ESPLength", 14, 1, 40, v -> targetEsp.is(Aura.ESP.FearClientV2) && page.is(Pages.Render)).addToGroup(espGroup);
    public final Setting<Integer> espFactor = new Setting<>("ESPFactor", 8, 1, 20, v -> targetEsp.is(Aura.ESP.FearClientV2) && page.is(Pages.Render)).addToGroup(espGroup);
    public final Setting<Float> espShaking = new Setting<>("ESPShaking", 1.8f, 1.5f, 10f, v -> targetEsp.is(Aura.ESP.FearClientV2) && page.is(Pages.Render)).addToGroup(espGroup);
    public final Setting<Float> espAmplitude = new Setting<>("ESPAmplitude", 3f, 0.1f, 8f, v -> targetEsp.is(Aura.ESP.FearClientV2) && page.is(Pages.Render)).addToGroup(espGroup);
