...
import fear.client.events.impl.PlayerUpdateEvent;
import fear.client.gui.notification.Notification;
import fear.client.injection.accesors.ILivingEntity;
...
import fear.client.utility.interfaces.IOtherClientPlayerEntity;
import fear.client.utility.math.MathUtility;
import fear.client.utility.player.InteractionUtility;
...
import fear.client.utility.render.animation.CaptureMark;
...
import static fear.client.utility.math.MathUtility.random;
...
public class Aura extends Module {
    ...
    public final Setting<ESP> esp = new Setting<>("ESP", ESP.FearClient);
    public final Setting<SettingGroup> espGroup = new Setting<>("ESPSettings", new SettingGroup(false, 0), v -> esp.is(ESP.FearClientV2));
    public final Setting<Integer> espLength = new Setting<>("ESPLength", 14, 1, 40, v -> esp.is(ESP.FearClientV2)).addToGroup(espGroup);
    public final Setting<Integer> espFactor = new Setting<>("ESPFactor", 8, 1, 20, v -> esp.is(ESP.FearClientV2)).addToGroup(espGroup);
    public final Setting<Float> espShaking = new Setting<>("ESPShaking", 1.8f, 1.5f, 10f, v -> esp.is(ESP.FearClientV2)).addToGroup(espGroup);
    public final Setting<Float> espAmplitude = new Setting<>("ESPAmplitude", 3f, 0.1f, 8f, v -> esp.is(ESP.FearClientV2)).addToGroup(espGroup);
    ...
    public void onUpdate(PlayerUpdateEvent e) {
        ...
        if (mc.player.isUsingItem() && pauseWhileEating.getValue())
            return;
        if(pauseBaritone.getValue() && FearClient.baritone){
    ...
    public float getAttackCooldownProgressPerTick() {
        return (float) (1.0 / mc.player.getAttributeValue(EntityAttributes.GENERIC_ATTACK_SPEED) * (20.0 * FearClient.TICK_TIMER * (tpsSync.getValue() ? Managers.SERVER.getTPSFactor() : 1f)));
    }
    ...
    public void onRender3D(MatrixStack stack) {
        ...
        switch (esp.getValue()) {
            ...
            case FearClientV2 -> Render3DEngine.renderGhosts(espLength.getValue(), espFactor.getValue(), espShaking.getValue(), espAmplitude.getValue(), target);
            case FearClient -> Render3DEngine.drawTargetEsp(stack, target);
        }
    ...
    public enum ESP {
        Off, FearClient, NurikZapen, CelkaPasta, FearClientV2
    }
