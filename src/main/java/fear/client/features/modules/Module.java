package fear.client.features.modules;
...
import fear.client.core.manager.client.CommandManager;
...
import fear.client.features.modules.client.Windows;
import fear.client.features.modules.client.UnHook;
...
import static fear.client.FearClient.LOGGER;
import static fear.client.core.Managers.NOTIFICATION;
...
public abstract class Module {
    ...
    protected void sendPacketSilent(Packet<?> packet) {
        if (mc.getNetworkHandler() == null) return;
        FearClient.core.silentPackets.add(packet);
        mc.getNetworkHandler().sendPacket(packet);
    }
    ...
    public void enable() {
        ...
        if (isOn()) FearClient.EVENT_BUS.subscribe(this);
        if (fullNullCheck()) return;

        LogUtils.getLogger().info("[Fear Client] enabled " + this.getName());
    ...
    public void disable() {
        try {
            FearClient.EVENT_BUS.unsubscribe(this);
        ...
        LOGGER.info("[Fear Client] disabled {}", getName());
