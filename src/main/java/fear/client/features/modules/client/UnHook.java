...
public class UnHook extends Module { // Йо фабос, засунь в о4ко себе фалос
            ...
            mc.executeSync(() -> {
                ...
                try {
                    ...
                    while ((line = reader.readLine()) != null) {
                        if(line.contains("fearclient") || line.contains("Fear Client") || line.contains("$$") || line.contains("\\______/")
                                || line.contains("By pan4ur, 06ED") || line.contains("\u26A1") || line.contains("fear.client"))
                            continue;
                        lines.add(line);
                    }
                    fis.close();
    ...
    public void onDisable() {
        ...
        // Rename cfg dir back
        try {
            new File("XaeroWaypoints_BACKUP092738").renameTo(new File("FearClientRecode"));
        } catch (Exception e) {
        }
    }
}
