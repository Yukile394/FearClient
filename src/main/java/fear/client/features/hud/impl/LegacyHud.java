...
import fear.client.gui.font.FontRenderer;
...
import fear.client.utility.math.FrameRateCounter;
...
import static fear.client.features.hud.impl.PotionHud.getDuration;
...
public class LegacyHud extends Module {
    ...
    public void onRender2D(DrawContext context) {
        ...
        if (waterMark.getValue())
            drawText(context, "fearclient v" + FearClient.VERSION, 2, waterMarkY.getValue());
        ...
        if (speed.getValue()) {
            String str = "Speed " + Formatting.WHITE + MathUtility.round(Managers.PLAYER.currentPlayerSpeed * (bps.getValue() ? 20f : 72f) * FearClient.TICK_TIMER) + (bps.getValue() ? " b/s" : " km/h");
            drawText(context, str, (width - getStringWidth(str) - 2), renderingUp.getValue() ? (height - 2 - (i += offset)) : (2 + i++ * offset));
        }
        if (chests.getValue()) {
