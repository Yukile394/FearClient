...
import static fear.client.core.manager.client.ConfigManager.IMAGES_FOLDER;
...
public final class ThunderUtility {
    ...
    public static @NotNull String getAuthors() {
        List<String> names = FearClient.MOD_META.getAuthors()
                .stream()
                .map(Person::getName)
                .toList();
    ...
    public static void syncVersion() {
        try {
            if (!new BufferedReader(new InputStreamReader(new URL("https://raw.githubusercontent.com/Yukile394/FearClient/main/version.txt").openStream())).readLine().equals(FearClient.VERSION))
                FearClient.isOutdated = true;
    ...
    public static void parseStarGazer() {
        ...
        try {
            for (int page = 1; page <= 3; page++) {
                URL url = new URL("https://api.github.com/repos/Yukile394/FearClient/stargazers?per_page=100&page=" + page);
    ...
    public static void syncContributors() {
        try {
            URL list = new URL("https://raw.githubusercontent.com/Yukile394/FearClient/main/team.txt");
            BufferedReader in = new BufferedReader(new InputStreamReader(list.openStream(), StandardCharsets.UTF_8));
            String inputLine;
            int i = 0;
            while ((inputLine = in.readLine()) != null) {
                FearClient.contributors[i] = inputLine.trim();
                i++;
            }
            in.close();
        } catch (Exception e) {
    ...
    public static void parseCommits() {
        try {
            URL url = new URL("https://api.github.com/repos/Pan4ur/FearClient-Recode/commits?per_page=50");
            BufferedReader in = new BufferedReader(new InputStreamReader(url.openStream(), StandardCharsets.UTF_8));

            changeLog.add("Changelog [Recode; Date: " + FearClient.BUILD_DATE + "; GitHash:" + FearClient.GITHUB_HASH + "]");
            changeLog.add("\n");
