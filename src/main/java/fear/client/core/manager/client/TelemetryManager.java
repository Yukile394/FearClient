...
public class TelemetryManager implements IManager {
    ...
    public void pingServer(String name) {
        HttpRequest req = HttpRequest.newBuilder(URI.create("https://api.fearclient.net/v1/users/online?name=" + name))
    ...
    public static List<String> getPlayers(boolean online) {
        final HttpRequest request = HttpRequest.newBuilder(URI.create("https://api.fearclient.net/v1/users" + (online ? "/online" : "")))
                .GET()
                .build();
