...
import fear.client.features.modules.client.Notifications;
...
public class NotificationManager implements IManager {
        ...
        if (SystemTray.isSupported()) {
            try {
                if (trayIcon == null) {
                    ...
                    trayIcon = new TrayIcon(image, "Fear Client");
                    trayIcon.setImageAutoSize(true);
                    trayIcon.setToolTip("Fear Client");
                    tray.add(trayIcon);
                }
    ...
    private void mac(final String message) {
        ...
        processBuilder.command("osascript", "-e", "display notification \"" + message + "\" with title \"FearClient\"");
        try {
    ...
    private void linux(final String message) {
        final ProcessBuilder processBuilder = new ProcessBuilder();
        processBuilder.command("notify-send", "-a", "Fear Client", message);
