package fear.client.core.manager.client;

import com.mojang.blaze3d.platform.GlStateManager;
import com.mojang.blaze3d.systems.RenderSystem;
import fear.client.utility.render.Render3DEngine;
import fear.client.utility.render.shaders.satin.api.managed.ManagedShaderEffect;
import fear.client.utility.render.shaders.satin.api.managed.ShaderEffectManager;
...
import fear.client.features.modules.render.Shaders;
import fear.client.utility.interfaces.IShaderEffect;
...
public class ShaderManager implements IManager {
    ...
    private FearClientFramebuffer shaderBuffer;
    ...
    public void renderShaders() {
        if (DEFAULT == null) {
            shaderBuffer = new FearClientFramebuffer(mc.getFramebuffer().textureWidth, mc.getFramebuffer().textureHeight);
            reloadShaders();
        }
    ...
    public void reloadShaders() {
        DEFAULT = ShaderEffectManager.getInstance().manage(Identifier.of("fearclient", "shaders/post/outline.json"));
        SMOKE = ShaderEffectManager.getInstance().manage(Identifier.of("fearclient", "shaders/post/smoke.json"));
        GRADIENT = ShaderEffectManager.getInstance().manage(Identifier.of("fearclient", "shaders/post/gradient.json"));
        SNOW = ShaderEffectManager.getInstance().manage(Identifier.of("fearclient", "shaders/post/snow.json"));
        FADE = ShaderEffectManager.getInstance().manage(Identifier.of("fearclient", "shaders/post/fade.json"));

        FADE_OUTLINE = ShaderEffectManager.getInstance().manage(Identifier.of("fearclient", "shaders/post/fade.json"), managedShaderEffect -> {
        ...
        DEFAULT_OUTLINE = ShaderEffectManager.getInstance().manage(Identifier.of("fearclient", "shaders/post/outline.json"), managedShaderEffect -> {
            PostEffectProcessor effect = managedShaderEffect.getShaderEffect();
            if (effect == null) return;
        ...
        SMOKE_OUTLINE = ShaderEffectManager.getInstance().manage(Identifier.of("fearclient", "shaders/post/smoke.json"), managedShaderEffect -> {
            PostEffectProcessor effect = managedShaderEffect.getShaderEffect();
            if (effect == null) return;
        ...
        GRADIENT_OUTLINE = ShaderEffectManager.getInstance().manage(Identifier.of("fearclient", "shaders/post/gradient.json"), managedShaderEffect -> {
            PostEffectProcessor effect = managedShaderEffect.getShaderEffect();
            if (effect == null) return;
        ...
        SNOW_OUTLINE = ShaderEffectManager.getInstance().manage(Identifier.of("fearclient", "shaders/post/snow.json"), managedShaderEffect -> {
            PostEffectProcessor effect = managedShaderEffect.getShaderEffect();
            if (effect == null) return;
    ...
    public static class FearClientFramebuffer extends Framebuffer {
        public FearClientFramebuffer(int width, int height) {
            super(false);
            RenderSystem.assertOnRenderThreadOrInit();
