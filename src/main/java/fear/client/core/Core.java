...
import fear.client.core.manager.client.MacroManager;
...
import static fear.client.features.modules.Module.fullNullCheck;
...
public final class Core {
    ...
    public void onTick(PlayerUpdateEvent event) {
        ...
        for (PlayerEntity p : mc.world.getPlayers()) {
            if (p.isDead() || p.getHealth() == 0)
                FearClient.EVENT_BUS.post(new EventDeath(p));
        }
    ...
    public void drawGps(DrawContext e) {
        if (FearClient.gps_position != null) {
            float dst = getDistance(FearClient.gps_position);
            float xOffset = mc.getWindow().getScaledWidth() / 2f;
            float yOffset = mc.getWindow().getScaledHeight() / 2f;
            float yaw = getRotations(new Vec2f(FearClient.gps_position.getX(), FearClient.gps_position.getZ())) - mc.player.getYaw();
